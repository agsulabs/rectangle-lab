...
def on_button_click(shape, overlay, menu_frame):
    clear_frame(menu_frame)
    shape_param(menu_frame, shape)
    close_menu(overlay)
        
def menu_button(overlay,list, menu_frame):
    ...
    for index, text in enumerate(list):
        ...
        button = tk.Button(
            overlay,
            text=text,
            command=lambda shape=text: on_button_click(shape, overlay, menu_frame),
            bg=menu_bg_color,
            fg=text_color,
            font=("Arial", 12, "bold")
        )   
...
def menu(window, menu_frame):
    ...
    menu_button(overlay, menu_list, menu_frame)

def shape_param(frame, shape = None):
    if shape == "Rechteck":
        ...
        frame.entry_width = tk.Entry(frame, width=8)
        ...
        frame.entry_height = tk.Entry(frame, width=8)
        ...
        frame.entry_left = tk.Entry(frame, width=8)
        ...
        frame.entry_right = tk.Entry(frame, width=8)
        ...
        frame.entry_base = tk.Entry(frame, width=8)
        ...
        frame.entry_radius = tk.Entry(frame, width=8)
        frame.entry_radius.grid(row=0, column=1, padx=5, pady=5)
        
def canvas_draw(canvas, shape = None):
...
def run_app():
    ...
    button = tk.Button(header_frame, text="Figur auswählen", command=lambda: menu(window, menu_frame), bg=accent_color, fg=body_bg_color, font=("Arial", 12, "bold"))
